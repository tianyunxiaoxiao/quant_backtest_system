...
- 省略 `--index` 的全 A 等权实际 CLI 回测通过：57 个交易日，账户恒等式显示残差
  `0.000000 bps`，产物 39 个（含新增 `position_period_analysis.parquet` 与
  `position_analysis_summary.json`）。
- 显式 `--index 000905.SH` 的中证 500 实际 CLI 回测通过：57 个交易日，账户恒等式
  显示残差 `0.000000 bps`，产物 39 个。
