...
失败的 `.partial` 候选目录和所有历史 release 都会保留，供诊断与恢复使用。每日更新流程不得
自动删除任何候选或历史行情版本。需要释放空间时，必须先只读列出精确目录、大小、引用关系、
备份与恢复方案，并在获得用户对本次具体删除对象的明确确认后单独执行。
